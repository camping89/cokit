...
SKILL_DIR=""
if [ -d "$HOME/.copilot/skills/chrome-devtools/scripts" ]; then
...
mkdir -p $SKILL_DIR/.copilot/chrome-devtools/tmp

# Write a test script
cat > $SKILL_DIR/.copilot/chrome-devtools/tmp/login-test.js << 'EOF'
...
# Run the test
node $SKILL_DIR/.copilot/chrome-devtools/tmp/login-test.js
```
