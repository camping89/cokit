...
Keep `%USERPROFILE%/.claude/` directory in version control:
...
├── .gitignore             # Ignore %USERPROFILE%/.claude/.env
└── README.md
```
...
Copy `%USERPROFILE%/.claude/.env.example` to `%USERPROFILE%/.claude/.env` and fill in:
- GITHUB_TOKEN
- DATABASE_URL
```
...
- `%USERPROFILE%/.claude/mcp.json` (without secrets)
- `%USERPROFILE%/.claude/.env.example`

**What NOT to commit:**
- `%USERPROFILE%/.claude/.env` (contains secrets)
- `%USERPROFILE%/.claude/memory/` (optional)
...
echo 'ANTHROPIC_API_KEY=sk-ant-xxxxx' > %USERPROFILE%/.claude/.env
```
...
```bash
# Check hooks.json
cat %USERPROFILE%/.claude/hooks.json | jq .

# Review scripts
cat %USERPROFILE%/.claude/scripts/hook.sh
...
```bash
#!/bin/bash
# %USERPROFILE%/.claude/scripts/rate-limit.sh

REQUESTS_FILE="%USERPROFILE%/.claude/requests.log"
MAX_REQUESTS=100
WINDOW=3600  # 1 hour
...
```markdown
# %USERPROFILE%/.claude/commands/test.md
...
```bash
# Create team plugin
cd %USERPROFILE%/.claude/plugins/team-plugin
cat > plugin.json <<EOF
{
  "name": "team-plugin",
  "skills": ["skills/*/"],
...
```bash
#!/bin/bash
# %USERPROFILE%/.claude/scripts/budget-check.sh
