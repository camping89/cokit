...
Hooks are configured in `%USERPROFILE%/.claude/hooks.json`:
...
```bash
#!/bin/bash
# %USERPROFILE%/.claude/scripts/validate-bash.sh
...
```json
{
  "hooks": {
    "pre-tool": {
      "bash": "%USERPROFILE%/.claude/scripts/validate-bash.sh"
    }
  }
}
```
...
```bash
#!/bin/bash
# %USERPROFILE%/.claude/scripts/format-code.sh
...
```json
{
  "hooks": {
    "post-tool": {
      "write": "%USERPROFILE%/.claude/scripts/format-code.sh",
      "edit": "%USERPROFILE%/.claude/scripts/format-code.sh"
    }
  }
}
```
...
```bash
#!/bin/bash
# %USERPROFILE%/.claude/scripts/track-usage.sh

# Log prompt
echo "$(date): $USER_PROMPT" >> %USERPROFILE%/.claude/usage.log
...
{
  "hooks": {
    "user-prompt-submit": "%USERPROFILE%/.claude/scripts/track-usage.sh"
  }
}
```
...
- Check script paths
- Review logs in `%USERPROFILE%/.claude/logs/`
