...
**Configuration**: `%USERPROFILE%/.claude/settings.json` (4-tier hierarchy)
...
3. **Project settings**: `%USERPROFILE%/.claude/settings.json` (team-shared, version controlled)
4. **Local settings**: `%USERPROFILE%/.claude/settings.local.json` (personal, git-ignored)
...
{
  "type": "command",
  "command": "jq -r '.tool_parameters.command' | tee -a %USERPROFILE%/.claude/commands.log"
}
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '.tool_parameters | \"\\(.command) - \\(.description)\"' >> %USERPROFILE%/.claude/commands.log"
          }
        ]
      }
    ]
...
Edit `%USERPROFILE%/.claude/settings.json` or `%USERPROFILE%/.claude/settings.local.json`:
...
**Check logs**:
```bash
cat %USERPROFILE%/.claude/logs/hooks.log
```
...
```bash
#!/bin/bash
CACHE_FILE="%USERPROFILE%/.claude/cache/format-cache"
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash %USERPROFILE%/.claude/scripts/validate-bash.sh"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash %USERPROFILE%/.claude/scripts/protect-files.sh"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash %USERPROFILE%/.claude/scripts/auto-format.sh"
          }
        ]
      }
    ],
    "Notification": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash %USERPROFILE%/.claude/scripts/desktop-notify.sh"
          }
        ]
      }
    ],
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash %USERPROFILE%/.claude/scripts/log-usage.sh"
          }
        ]
      }
    ],
