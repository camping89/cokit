...
3. **Project settings** (`%USERPROFILE%/.claude/settings.json`)
...
```json
{
  "logging": {
    "level": "info",
    "file": "%USERPROFILE%/.claude/logs/session.log",
    "console": true
  }
}
```
...
### Project Settings
- Keep project-specific in `%USERPROFILE%/.claude/settings.json`
...
cat %USERPROFILE%/.claude/settings.json | jq .
