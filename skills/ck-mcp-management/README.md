...
```bash
cd %USERPROFILE%/.claude/skills/mcp-management/scripts
npm install
```

### 2. Configure MCP Servers

Create `%USERPROFILE%/.claude/.mcp.json`:
...
See `%USERPROFILE%/.claude/.mcp.json.example` for more examples.
...
- Load config from `%USERPROFILE%/.claude/.mcp.json`
...
2. `%USERPROFILE%/.claude/skills/mcp-management/.env`
3. `%USERPROFILE%/.claude/skills/.env`
4. `%USERPROFILE%/.claude/.env`
...
The `mcp-manager` agent (`%USERPROFILE%/.claude/agents/mcp-manager.md`) uses this skill to:
...
Ensure `%USERPROFILE%/.claude/.mcp.json` exists and is valid JSON.
