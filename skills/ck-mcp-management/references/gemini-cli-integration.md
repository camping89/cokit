...
Gemini CLI reads MCP servers from `.gemini/settings.json`. Create a symlink to `%USERPROFILE%/.claude/.mcp.json`:
...
ln -sf %USERPROFILE%/.claude/.mcp.json .gemini/settings.json
...
1. **Configuration Loading**: Reads `.gemini/settings.json` (symlinked to `%USERPROFILE%/.claude/.mcp.json`)
...
Edit `%USERPROFILE%/.claude/.mcp.json`:
...
Should show symlink pointing to `%USERPROFILE%/.claude/.mcp.json`.
