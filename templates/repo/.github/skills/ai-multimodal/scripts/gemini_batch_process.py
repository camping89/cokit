...
def find_api_key() -> Optional[str]:
    ...
    Uses ~/.copilot/scripts/resolve_env.py for consistent resolution across all skills.
    Falls back to local resolution if centralized resolver not available.

    Priority order (highest to lowest):
    1. process.env (runtime environment variables)
    2. PROJECT/.copilot/skills/ai-multimodal/.env (skill-specific)
    3. PROJECT/.copilot/skills/.env (shared skills)
    4. PROJECT/.copilot/.env (project global)
    5. ~/.copilot/skills/ai-multimodal/.env (user skill-specific)
    6. ~/.copilot/skills/.env (user shared)
    7. ~/.copilot/.env (user global)
    """
    ...
    try:
        ...
        for i, generated_image in enumerate(response.generated_images):
            ...
            for parent in [script_dir] + list(script_dir.parents):
                if (parent / '.git').exists() or (parent / '.copilot').exists():
                    project_root = parent
                    break
                ...
                for i, part in enumerate(response.candidates[0].content.parts):
                    if part.inline_data:
                        ...
                        if file_path:
                            ...
                            # Find project root (look for .git or .copilot directory)
                            script_dir = Path(__file__).parent
                            project_root = script_dir
                            for parent in [script_dir] + list(script_dir.parents):
                                if (parent / '.git').exists() or (parent / '.copilot').exists():
...
def batch_process(
    ...
    if not api_key:
        ...
        print("2. Show hierarchy: python ~/.copilot/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal")
        print("3. Quick setup: export GEMINI_API_KEY='your-key'")
        print("4. Create .env: cd ~/.copilot/skills/ai-multimodal && cp .env.example .env")
