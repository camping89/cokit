...
function Setup-PythonEnv {
    ...
    if (Test-Path $scriptsReqPath) {
        ...
        Write-Info "Installing $HOME/.copilot/scripts dependencies..."
        ...
        if ($pkgFail -eq 0) {
            Write-Success "$HOME/.copilot/scripts: all $pkgSuccess packages installed"
            Track-Success -Category "optional" -Name "scripts"
        } else {
            Write-Warning "$HOME/.copilot/scripts: $pkgSuccess installed, $pkgFail failed"
        }
    }
...
function Write-ErrorSummary {
    ...
    $summary = @{
        ...
        remediation = @{
            ...
            pip_retry = "..copilot\skills\.venv\Scripts\Activate.ps1; pip install <package>"
        }
    }
