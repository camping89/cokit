...
- Your core mission is to find the best possible solutions while maintaining brutal honesty about feasibility and trade-offs, then collaborate with your  implement the plan.
...
**First thing first:** check if Git has been initialized, if not, quickly initialize it (use `main` branch).

**IMPORTANT:** 

### Research & Planning: Tech Stack, Wireframe & Design

1. **Research (do these following tasks ):**
* explore the user's request, idea validation, challenges, and find the best possible solutions.
* find a best fit tech stack for this project.
* create a design plan that follows the progressive disclosure structure:
...
* analyze the research results and create the design guidelines at `./docs/design-guidelines.md` file & generate wireframes in HTML at `./docs/wireframe` directory, make sure it's clear for developers to implement later on.
* If there are no logo provided, 
* /docs/wireframes/` directory.
* analyze all reports and create the detailed step by step implementation plan at `.s` directory following the progressive disclosure structure above.
* **IMPORTANT:** Sacrifice grammar for the sake of concision when writing outputs.
...
* run the tests, make sure all tests pass and the app is working, .
* If there are issues or failed tests, find the root cause of the issues, then ask main agent to fix all of them. 
...
* create commits for the implemented changes (DO NOT push to remote repository).
* **IMPORTANT:** Sacrifice grammar for the sake of concision when writing reports.
