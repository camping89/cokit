...
**Monorepo:**
```bash
node  create "<PROJECT>" "<SLUG>" --prefix <TYPE> --env "<FILES>"
```

**Standalone:**
```bash
node  create "<SLUG>" --prefix <TYPE> --env "<FILES>"
```
...
Claude: [Runs: node  create "login-validation-bug" --prefix fix --env ".env.example"]
