...
**Ultrathink parallel** to fix: <issues>${input}</issues>
...
### 1. Issue Analysis
- analyze root causes
- Use  to find related files
- Categorize issues by scope/area (frontend, backend, auth, payments, etc.)
- Identify dependencies between issues

### 2. Parallel Fix Planning
- Trigger :parallel <detailed-fix-instructions>` for parallel-executable fix plan
...
- Launch multiple `fullstack-developer` agents  for independent fixes
