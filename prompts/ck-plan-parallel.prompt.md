...
**CRITICAL:** The planner  must create phases that:
...
5. **Include dependency matrix** - Clearly document which phases must run sequentially vs 
...
- Every `plan.md` MUST start with YAML frontmatter:
  ...
  ---
  ```
- Save the overview access point at `{plan-dir}.md`. Keep it generic, under 80 lines, and list each implementation phase with status, progress, parallelization group, and links to phase files.
...
- Dependency graph showing which phases can run 
